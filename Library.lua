local Arcane = {}
local Library = Arcane

if not getgenv().Arcane then
...
Library.IsMobileDevice = IsMobileDevice
...
function Library:GetTweenProperty(RawItem)
    local Object = RawItem or Self.Instance
...
function Library:Fade(Property, Visibility, RawItem)
    local Object = RawItem or Self.Instance
...
function Library:FadeDescendants(Visibility, Callback)
    if Visibility then
...
function Library:ChangeItemTheme(Properties)
    local Object = Self.Instance
...
function Library:OnHover(OnEnter, OnLeave)
    local Object = Self.Instance
...
function Library:Notification(Params)
    ...
    local TitleX = 12
    local IconSize = IsMobile and 16 or 18
    ...
    if Icon then
        ...
        TitleX = IsMobile and 34 or 38
    end

    local TextSize = IsMobile and 13 or 14
...
function Library:Window(Params)
    ...
    local Scale = Library:GetScreenScale()
    local Viewport = workspace.CurrentCamera.ViewportSize
    local WindowWidth = IsMobile and 350 or 526
    local WindowHeight = IsMobile and 420 or 515

    Items.MainFrame = Library:Create("Frame", {
        Parent = Library.Holder.Instance,
        Name = "\0",
        AnchorPoint = Vector2.new(0, 0),
        Position = UDim2.fromOffset(Viewport.X / (2 * Scale) - WindowWidth/2, Viewport.Y / (2 * Scale) - WindowHeight/2),
    ...
    local ThemeWidth = IsMobile and 80 or 96
    ...
    Items.ThemeHolder = Library:Create("Frame", {
        ...
        Size = UDim2.new(0, ThemeWidth, 0, 40),
        ZIndex = 3,
        BorderSizePixel = 0,
        BackgroundColor3 = Library.Theme.Topbar
    }):AddToTheme({ BackgroundColor3 = "Topbar" })
    ...
    function Window:Center()
        ...
        local WW = IsMobile and 350 or 526
        local WH = IsMobile and 420 or 515
        Items.MainFrame.Instance.Position = UDim2.fromOffset(Vp.X / (2 * CScale) - WW/2, Vp.Y / (2 * CScale) - WH/2)
    end
...
function Library:Page(Params)
    ...
    local TopGap = IsMobile and 100 or 110
    local BottomGap = IsMobile and 166 or 176

    Items.Columns = Library:Create("Frame", {
        Parent = Items.Content.Instance,
        Name = "\0",
        BackgroundTransparency = 1,
        Position = UDim2.new(0, 15, 0, TopGap),
        Size = UDim2.new(1, -30, 1, -BottomGap),
        BorderSizePixel = 0
    })
...
function Library:SubPage(Params)
    ...
    local ColumnWidth = IsMobile and 1 or 0.5
    local ColumnGap = IsMobile and 0 or -8
    ...
    Items.Left = Library:Create("ScrollingFrame", {
        ...
        Position = UDim2.new(0, 0, 0, 0),
        Size = UDim2.new(ColumnWidth, ColumnGap, 1, 0),
        ZIndex = 2,
        BorderSizePixel = 0
    })
...
function Library:Toggle(Params)
    ...
    local ToggleSize = IsMobile and 24 or 28
    local CircleSize = IsMobile and 7 or 8
    ...
    Items.Background = Library:Create("Frame", {
        ...
        Size = UDim2.new(0, ToggleSize, 0, ToggleSize/2),
        BorderSizePixel = 0,
    ...
    Library:Create("UICorner", {
        ...
        CornerRadius = UDim.new(0, ToggleSize/2)
    })
    ...
    function Toggle:Set(Bool)
        ...
        local CircleOffset = ToggleSize - CircleSize - 6
        ...
        if Bool then
            ...
            Items.Circle:Tween({ BackgroundColor3 = Library.Theme.AccentDark, Position = UDim2.new(0, CircleOffset, 0, 3) })
        else
...
function Library:Slider(Params)
    ...
    local DragSize = IsMobile and 18 or 14
    ...
    Items.Drag = Library:Create("Frame", {
        ...
        Size = UDim2.new(0, DragSize, 0, IsMobile and 10 or 8),
        BorderSizePixel = 0
    })
    ...
    Items.Input:Connect("InputBegan", function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            Slider.Sliding = true
            Items.Drag:Tween({ Size = UDim2.new(0, IsMobile and 22 or 18, 0, IsMobile and 14 or 12) }, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out))
            ...
            InputChanged = Input.Changed:Connect(function()
                if Input.UserInputState == Enum.UserInputState.End then
                    ...
                    Items.Drag:Tween({ Size = UDim2.new(0, IsMobile and 18 or 14, 0, IsMobile and 10 or 8) }, TweenInfo.new(0.15, Enum.EasingStyle.Quart, Enum.EasingDirection.Out))
                    InputChanged:Disconnect()
                    InputChanged = nil
                end
            end)
...
function Library:Dropdown(Params)
    ...
    Items.Opened = Library:Create("Frame", {
        ...
        Visible = false,
        ClipsDescendants = true,
        Size = UDim2.new(0, IsMobile and 180 or 218, 0, 0),
        ZIndex = 50,
        BorderSizePixel = 0,
        BackgroundColor3 = Library.Theme.DropdownBack
    }):AddToTheme({ BackgroundColor3 = "DropdownBack" })
...
function Library:List(Params)
    ...
    Items.Box = Library:Create("Frame", {
        ...
        Size = UDim2.new(1, -24, 0, IsMobile and 150 or 138),
        BorderSizePixel = 0,
        BackgroundColor3 = Library.Theme.DropdownBack
    }):AddToTheme({ BackgroundColor3 = "DropdownBack" })
...
function Library:Colorpicker(Params)
    ...
    Items.Swatch = Library:Create("TextButton", {
        ...
        AnchorPoint = Vector2.new(1, 0.5),
        Position = UDim2.new(1, -12, 0.5, 0),
        Size = UDim2.new(0, IsMobile and 40 or 34, 0, IsMobile and 20 or 16),
        BorderSizePixel = 0,
    ...
    local WindowSize = IsMobile and 190 or 210

    Items.Window = Library:Create("Frame", {
        Parent = Library.UnusedHolder.Instance,
        Name = "\0",
        Visible = false,
        Size = UDim2.new(0, WindowSize, 0, WindowSize),
        ZIndex = 50,
        BorderSizePixel = 0,
        BackgroundColor3 = Library.Theme.Section
    }):AddToTheme({ BackgroundColor3 = "Section" })
    ...
    local PaletteHeight = IsMobile and 90 or 108
    ...
    Items.Palette = Library:Create("ImageButton", {
        ...
        Size = UDim2.new(1, -24, 0, PaletteHeight),
        ZIndex = 51,
        BorderSizePixel = 0,
    ...
    Items.Hue = Library:Create("ImageButton", {
        ...
        AutoButtonColor = false,
        Position = UDim2.new(0, 12, 0, PaletteHeight + 20),
    ...
    Items.Alpha = Library:Create("ImageButton", {
        Parent = Items.Window.Instance, Name = "\0",
        AutoButtonColor = false,
        Position = UDim2.new(0, 12, 0, PaletteHeight + 40),
        Size = UDim2.new(1, -54, 0, 14),
        ZIndex = 51, BorderSizePixel = 0,
        BackgroundColor3 = Colorpicker.Default
    })
    ...
    local CopySize = IsMobile and 34 or 28

    Items.Copy = Library:Create("TextButton", {
        Parent = Items.Window.Instance, Name = "\0",
        Text = "", AutoButtonColor = false,
        AnchorPoint = Vector2.new(1, 0.5),
        Position = UDim2.new(1, -8, 0, PaletteHeight + 40),
        Size = UDim2.new(0, CopySize, 0, IsMobile and 44 or 40),
        ZIndex = 51, BorderSizePixel = 0,
        BackgroundColor3 = Library.Theme.DropdownBack
    }):AddToTheme({ BackgroundColor3 = "DropdownBack" })
    ...
    Items.Hex = Library:Create("TextBox", {
        ...
        TextColor3 = Library.Theme.Text,
        PlaceholderColor3 = Library.Theme.DimText,
        PlaceholderText = "#ffffff", Text = "#ffffff", TextSize = IsMobile and 12 or 14,
        ClearTextOnFocus = false, CursorPosition = -1,
        Position = UDim2.new(0, 12, 0, PaletteHeight + 60),
    ...
    Items.Paste = Library:Create("ImageButton", {
        ...
        AnchorPoint = Vector2.new(1, 0.5),
        BackgroundTransparency = 1,
        Position = UDim2.new(1, -12, 0, PaletteHeight + 73),
    ...
    local function PopupPosition(ExtraY)
        local Scale = Library:GetScreenScale()
        return UDim2.fromOffset(Items.Swatch.Instance.AbsolutePosition.X / Scale - (IsMobile and 160 or 176), (Items.Swatch.Instance.AbsolutePosition.Y + Items.Swatch.Instance.AbsoluteSize.Y + GuiInset + (ExtraY or 0)) / Scale)
    end
